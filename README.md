...
it is confident (visa can only fail this way, since its `unknown` means the
employer is not on the IND register), leaves it `unknown` with the probability
noted when it is not,
